...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument("--max-workers-cap", type=int, default=16, help="Cap for parallel workers")
...
def run_experiment(
    ...
    # --- Branch for LSTM ---
        ...
        if n_samples == 0:
            logging.error("[%s] No LSTM training windows; aborting.", experiment_name)
            return None

        tscv_local = TimeSeriesSplit(n_splits=3)
        n_features = X_all.shape[2]
        # grid to evaluate (small, product of lists)
        grid_space = {
            'units': [100, 500, 700],
            'dropout1': [0.0, 0.1, 0.2],
        ...
        param_list = list(ParameterGrid(grid_space))
        logging.info("[%s] Parameter grid size: %d combinations", experiment_name, len(param_list))
        ...
        def eval_fold(params, train_idx, val_idx, seed_base):
            # deterministic per-eval seed
            seed_local = seed_base + (
                    int(hashlib.md5(json.dumps(params, sort_keys=True).encode()).hexdigest(), 16) % 10_000)
            set_seeds(seed_local)
            ...
            # fit scalers on fold's train
            scalers_fold = [StandardScaler() for _ in range(n_features)]
            for j in range(n_features):
                scalers_fold[j].fit(X_tr_fold[:, :, j].reshape(-1, 1))

            def transform_fold(X):
                out = np.empty_like(X, dtype=np.float32)
                for j in range(n_features):
                    col = X[:, :, j].reshape(-1, 1)
                    out[:, :, j] = scalers_fold[j].transform(col).reshape(X.shape[0], X.shape[1])
                return out

            X_tr_s = transform_fold(X_tr_fold)
            X_val_s = transform_fold(X_val_fold)

            scaler_y_fold = MinMaxScaler()
            scaler_y_fold.fit(y_tr_fold)
            y_tr_s = scaler_y_fold.transform(y_tr_fold)
            y_val_s = scaler_y_fold.transform(y_val_fold)

            # build model for this fold
            try:
                model = build_lstm(input_shape=(args.timesteps - 1, n_features),
                                   units=int(params['units']),
                                   dropout1=float(params['dropout1']),
                                   recurrent_dropout1=float(params.get('recurrent_dropout1')),
                                   dropout2=float(params['dropout2']),
                                   recurrent_dropout2=float(params.get('recurrent_dropout2')),
                                   lr=float(params['lr']),
            ...
            # callbacks (short patience to speed CV)
            cb = [
                keras.callbacks.EarlyStopping(monitor='val_loss', patience=15, start_from_epoch=25, restore_best_weights=True, verbose=0),
                #keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=4, min_lr=1e-4, verbose=0)
            ]

            # fit
            try:
                hist = model.fit(X_tr_s, y_tr_s, validation_data=(X_val_s, y_val_s),
                                 epochs=args.epochs, batch_size=int(params['batch_size']),
                ...
                # clear TF session to free GPU memory inside this process
                try:
        ...
        def evaluate_params(params):
            ...
            fold_losses = []
            # compute train/val splits (TimeSeriesSplit yields indices on windowed arrays)
            for fold_i, (train_idx, val_idx) in enumerate(tscv_local.split(X_all)):
                loss = eval_fold(params, train_idx, val_idx, seed_base + fold_i * 7)
                fold_losses.append(loss)
            mean_loss = float(np.mean(fold_losses)) if len(fold_losses) > 0 else np.inf
        ...
        # Run parallel evaluation over param_list (parallel over param sets, sequential CV inside each process)
        results_list = []

        # with parallel_backend('loky', n_jobs=n_jobs//2):
        #     with threadpool_limits(limits=threads_per_worker):
        #         from joblib import Parallel, delayed
        #         logging.info("[%s] Starting parallel param evaluation with n_jobs=%d threads_per_worker=%d",
        #                      experiment_name, n_jobs, threads_per_worker)
        #         results_list = Parallel(n_jobs=n_jobs, verbose=10)(
        #             delayed(evaluate_params)(p) for p in param_list
        #        )
        for p in param_list:
            results_list.append(evaluate_params(p))
        ...
        # Retrain final model on full training set (X_all, y_all) with best_params
        # Fit scalers on full training windows
        scalers_X = [StandardScaler() for _ in range(n_features)]
        for j in range(n_features):
            scalers_X[j].fit(X_all[:, :, j].reshape(-1, 1))

        def transform_full(X):
            out = np.empty_like(X, dtype=float)
            for j in range(n_features):
                out[:, :, j] = scalers_X[j].transform(X[:, :, j].reshape(-1, 1)).reshape(X.shape[0], X.shape[1])
            return out

        scaler_y_full = MinMaxScaler()
        scaler_y_full.fit(y_all)

        X_all_s = transform_full(X_all)
        X_test_s = transform_full(X_test) if X_test.shape[0] > 0 else np.empty((0, args.timesteps - 1, n_features))
        y_all_s = scaler_y_full.transform(y_all)

        # build final model and train
        set_seeds(args.seed)
        final_model = build_lstm(input_shape=(args.timesteps - 1, n_features),
                                 units=int(best_params['units']),
                                 dropout1=float(best_params['dropout1']),
                                 recurrent_dropout=float(best_params.get('recurrent_dropout1', 0.0)),
                                 dropout2=float(best_params['dropout2']),
                                 recurrent_dropout2=float(best_params.get('recurrent_dropout2', 0.0)),
                                 activation='relu',
                                 lr=float(best_params['lr']))

        callbacks_final = [
            keras.callbacks.EarlyStopping(monitor='loss', patience=15,  start_from_epoch=25, restore_best_weights=True, verbose=0),
            #keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=4, min_lr=1e-4, verbose=0)
        ]

        final_model.fit(X_all_s, y_all_s, epochs=args.lstm_epochs, batch_size=int(best_params['batch_size']),
                        callbacks=callbacks_final, verbose=1 if args.verbose else 0)

        # predictions & inverse transforms
        y_train_pred_s = final_model.predict(X_all_s, batch_size=int(best_params['batch_size']), verbose=0)
        y_train_pred = scaler_y_full.inverse_transform(y_train_pred_s)
        y_train_true = scaler_y_full.inverse_transform(y_all_s)

        if X_test_s.shape[0] > 0:
            y_test_pred_s = final_model.predict(X_test_s, batch_size=int(best_params['batch_size']), verbose=0)
            y_test_pred = scaler_y_full.inverse_transform(y_test_pred_s)
            y_test_true = scaler_y_full.inverse_transform(scaler_y_full.transform(y_test)) if y_test.shape[
                                                                                                  0] > 0 else np.empty(
                (0, 1))
        else:
        ...
        joblib.dump(scalers_X, prefix / "scalers_X.pkl")
        joblib.dump(scaler_y_full, prefix / "scaler_y.pkl")
